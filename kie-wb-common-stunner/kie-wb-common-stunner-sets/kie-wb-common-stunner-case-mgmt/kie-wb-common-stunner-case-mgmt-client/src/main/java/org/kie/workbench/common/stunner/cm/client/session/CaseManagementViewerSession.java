...
import org.kie.workbench.common.stunner.core.client.session.impl.ViewerSessionDelegate;
...
public class CaseManagementViewerSession
        extends ViewerSessionDelegate<ViewerSession> {

    private final DefaultViewerSession session;

    @Inject
    public CaseManagementViewerSession(final DefaultViewerSession session) {
    ...
    public void load(final Metadata metadata,
        ...
        session.load(s -> s.registerCanvasControl(ZoomControl.class)
    ...
    @Override
    protected ViewerSession getDelegate() {
        return session;
    }
}
