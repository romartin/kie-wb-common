...
public class ClientFormGenerationManagerTest {
    ...
    public void testOnFormGenerationSuccess() {
        ...
        verify(translationService, times(1)).getKeyValue(eq(FormsClientConstants.FormsGenerationSuccess),
                                                         eq("name1"));
        verify(messageNotifier, times(1)).accept(anyString());
        verify(errorNotifier, never()).accept(anyString());
    }
    ...
    public void testOnFormGenerationFailure() {
        ...
        verify(translationService, times(1)).getKeyValue(eq(FormsClientConstants.FormsGenerationFailure),
                                                         eq("name1"));
        verify(errorNotifier, times(1)).accept(anyString());
        verify(messageNotifier, never()).accept(anyString());
    }
}
